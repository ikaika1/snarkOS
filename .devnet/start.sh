...
start_snarkos_in_tmux() {
  ...
  ssh -o StrictHostKeyChecking=no aws-n$NODE_ID << EOF
    ...
    tmux send-keys -t "snarkos-session" "snarkos start --nodisplay --bft 0.0.0.0:5000 --rest 0.0.0.0:3033 --peers 47.242.151.228:4149,211.171.42.231:4133210.106.251.155:4133 --validators $NODE_IP:5000 --verbosity $VERBOSITY --dev $NODE_ID --dev-num-validators $NUM_INSTANCES --validator" C-m
